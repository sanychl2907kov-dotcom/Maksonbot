...
c.execute('''CREATE TABLE IF NOT EXISTS rules_channels (
...
def db_get_rules_channel(type_name):
    c.execute("SELECT channel_id FROM rules_channels WHERE type = ?", (type_name,))
    row = c.fetchone()
    return int(row[0]) if row else None

def db_set_rules_channel(type_name, channel_id):
    c.execute("INSERT OR REPLACE INTO rules_channels (type, channel_id) VALUES (?, ?)", (type_name, str(channel_id)))
    conn.commit()
...
async def create_rules_thread(interaction):
    ...
    try:
        ...
        # ========== ОТПРАВЛЯЕМ ТВОИ ПРАВИЛА ИЗ RULES_DICT ==========
        embed = discord.Embed(
            title="📋 Правила сервера",
            description="\n\n".join(RULES_DICT.values()),
            color=discord.Color.gold()
...
class RulesButton(Button):
    ...
    async def callback(self, i: discord.Interaction):
        ...
        try:
            ...
            if thread:
                await i.followup.send(
                    f"✅ Правила уже созданы в ветке: {thread.mention}\n"
                    f"Нажмите на ссылку, чтобы перейти.",
                    ephemeral=True
                )
            else:
...
@bot.tree.command(name="set_rules_channel", description="Установить канал для правил")
@app_commands.default_permissions(administrator=True)
async def set_rules_channel(i: discord.Interaction, channel: discord.TextChannel, type: str = "rules"):
    await i.response.defer(ephemeral=True)
    
    if type not in ["rules", "commands"]:
        await i.followup.send("❌ Тип должен быть 'rules' или 'commands'", ephemeral=True)
        return
    
    db_set_rules_channel(type, channel.id)
    await i.followup.send(f"✅ Канал {channel.mention} установлен как {type}", ephemeral=True)
...
async def commands_cmd(i: discord.Interaction):
    embed = discord.Embed(
        ...
        description=(
            ...
            "/set_rules_channel <канал> [тип] - Установить канал для правил\n"
